...
// Helper: determine if a trade show is in the past (beyond its end date)
const isPastTradeShow = (tradeShow: TradeShow): boolean => {
  if (!tradeShow.endDate) {
    return false;
  }
  ...
  const end = new Date(tradeShow.endDate);
...
const TradeShows = () => {
  ...
  // Split shows into current/upcoming vs history (past end date)
  const activeShows = tradeShows.filter((ts) => !isPastTradeShow(ts));
  const pastShows = tradeShows.filter((ts) => isPastTradeShow(ts));
